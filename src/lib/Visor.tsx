...
function Visor(props: VisorProps) {
    ...
    const onPinned = (key: string) => { props.onPinned(key); }
    const onClosed = (key: string) => { props.onClosed(key); }

    const GenerateElement = (el: ReactGridLayoutComponent) => {
        return (
            <div key={el.i} data-grid={{ x: el.x, y: el.y, w: el.w, h: el.h, minH: el.h, minW: el.w, static: el.static }} >
                <Window title={el.i} child={el.child} layout={el} onPinned={onPinned} onClosed={onClosed}></Window>
            </div>
        );
    }
    ...
    return (
        <div className="visor-container">
            <ResponsiveReactGridLayout className="layout" rowHeight={30} cols={cols} draggableHandle={".draggable"} isBounded={true} resizeHandle={<BottomRightHandle></BottomRightHandle>}>
